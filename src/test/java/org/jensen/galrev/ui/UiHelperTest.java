...
public class UiHelperTest {
    ...
    public void testFillTreeItemEmpty() throws Exception {
        ReviewTreeEntry dp = new ReviewTreeEntry(null, Paths.get("."));
        TreeItem<ReviewTreeEntry> root = new TreeItem<>(dp);
    ...
    public void testFillTreeItemDirs() throws Exception {
        ...
        TreeItem<ReviewTreeEntry> subItem = getTreeItemByPath(root, "/a/b");
    ...
    public void testFillTreeItemMissingIntermediateDirs() throws Exception {
        ...
        TreeItem<ReviewTreeEntry> subItem = getTreeItemByPath(root, "/a");
    ...
    private void printTree(TreeItem<ReviewTreeEntry> root, int indent) {
        String prefix = "";
        for (int i = 0; i < indent; i++) {
            prefix += " ";
        }
        String dispString;
        ReviewTreeEntry dp = root.getValue();
        if (dp.getPath() != null) {
    ...
    private TreeItem<ReviewTreeEntry> getTreeItemByPath(TreeItem<ReviewTreeEntry> root, String path) {
    ...
    private void testItemListContent(ObservableList<TreeItem<ReviewTreeEntry>> treeItems, String... paths) {
        if (paths == null || paths.length == 0) {
            assertTrue(treeItems.isEmpty());
        } else {
            assertEquals(paths.length, treeItems.size());
            List<String> pathList = Arrays.asList(paths);
            Optional<TreeItem<ReviewTreeEntry>> wrongEntry =
                    treeItems.stream()
    ...
    private void testItemFileContent(TreeItem<ReviewTreeEntry> subItem, String... filenames) {
        List<ReviewTreeEntry> children = subItem.getChildren().stream().map(TreeItem::getValue).collect(Collectors.toList());
        ...
        Optional<ReviewTreeEntry> wrongEntry = children.stream()
                .filter(dp -> ((dp.getImageFile() != null) &&
                        (!filenameList.contains(dp.getFileName()))))
                .findAny();
        assertFalse("Did find this one although not expected: " + wrongEntry, wrongEntry.isPresent());
    }
